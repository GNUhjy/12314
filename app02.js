...
io.on('connection', (socket) => {
    ...
    socket.on('connectConsumerTransport', async ({ dtlsParameters, id}, callback) => {
        ...
        try {
            ...
            if(id === consumertransport[0].id){
                consumertransport[0].connect({ dtlsParameters });
                console.log(socket.id, ' : audio를 connect')
            } else if(id === consumertransport[1].id){
                consumertransport[1].connect({ dtlsParameters });
