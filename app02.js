...
io.on('connection', (socket) => {
    ...
    socket.on('createProducerTransport', async (_, callback) => {
        console.log(socket.id,' : Producertransport 생성을 요청.');
        ...
        try {
            ...
            console.log(socket.id, ' : Producertransport 생성 완료.');
            ...
            console.error(socket.id, ' : transport 생성 실패. 클라이언트 ', error);
    ...
    socket.on('createConsumerTransport', async (_, callback) => {
        ...
        try {
            ...
            console.log(socket.id, ' : consumer 생성 완료');
    ...
    socket.on('connectProducerTransport', async ({ dtlsParameters }, callback) => {
        ...
        try {
            ...
            if (!producerTransport) {
                throw new Error(socket.id, ' : 해당 클라이언트에 연결된 producerTransport가 없습니다');
            }
    ...
    socket.on('connectConsumerTransport', async ({ dtlsParameters }, callback) => {
        ...
        try {
            ...
            if (!consumerTransport) {
                throw new Error(socket.id, ' : Consumer Transport가 존재하지 않습니다: ');
            }
            if (consumerTransport._isConnected){
                console.log(socket.id,' : 이 consumer는 이미 연결됨');
                return callback()
            }
    ...
    socket.on('produce', async ({ kind, rtpParameters }, callback) => {
        ...
        try {
            ...
            try{
                ...
                console.log(socket.id, ' : ', kind, ' 을 받음');
    ...
    socket.on('consume', async ({ producerId, kind, rtpCapabilities }, callback) => {
        console.log(socket.id, ' : consume 요청');
        ...
        try {
            ...
            console.log(socket.id, ' : consume 확인 및 시작');
